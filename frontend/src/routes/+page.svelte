<script lang="ts">
	...
	async function createFramily() {
		...
		try {
			...
			const response = await api.framily.create(newFramilyName || undefined, token);
			// Connect to the newly created framily
			await api.framily.connect(response.framily_code, token);
			showCreateForm = false;
			newFramilyName = '';
			await loadFramilies();
		} catch (e: any) {
			formError = e.message || 'Failed to create framily';
		} finally {
			formLoading = false;
		}
	}
...
<div class="dashboard">
	<h1>Welcome, {$authStore.user?.display_name || $authStore.user?.username || 'User'}!</h1>
	...
	{:else}
				...
				<div class="framily-grid">
					{#each pendingInvitations as invite}
						<div class="framily-card invitation">
							<h3>{invite.name || 'Unnamed Framily'}</h3>
							<p class="code">Code: {invite.code}</p>
							<p class="members">{invite.member_count} member{invite.member_count !== 1 ? 's' : ''}</p>
							<div class="invitation-actions">
								<button class="accept-btn" onclick={() => handleInvitation(invite.code, true)}>Accept</button>
								<button class="decline-btn" onclick={() => handleInvitation(invite.code, false)}>Decline</button>
							</div>
						</div>
					{/each}
				</div>
		...
		<section class="my-framilies">
			<div class="section-header">
				<h2>My Framilies</h2>
				<div class="actions">
					<button onclick={() => { showCreateForm = true; showConnectForm = false; }}>Create Framily</button>
					<button onclick={() => { showConnectForm = true; showCreateForm = false; }}>Connect to Framily</button>
				</div>
			</div>
			...
			{#if showCreateForm}
				...
				<div class="modal-overlay" onclick={() => showCreateForm = false}>
					...
					<div class="modal" onclick={(e) => e.stopPropagation()}>
						<h3>Create New Framily</h3>
						<form onsubmit={(e) => { e.preventDefault(); createFramily(); }}>
							<input 
								type="text" 
								placeholder="Framily Name (optional)" 
								bind:value={newFramilyName}
							/>
							{#if formError}
			...
			{#if showConnectForm}
				...
				<div class="modal-overlay" onclick={() => showConnectForm = false}>
					...
					<div class="modal" onclick={(e) => e.stopPropagation()}>
						...
						<form onsubmit={(e) => { e.preventDefault(); connectToFramily(); }}>
							<input 
								type="text" 
							...
							{/if}
							<div class="modal-actions">
								<button type="button" onclick={() => showConnectForm = false}>Cancel</button>
								<button type="submit" disabled={formLoading || connectCode.length !== 8}>
			...
			{#if framilies.length === 0}
				<p class="no-framilies">You haven't joined any framilies yet. Create one or connect using a code!</p>
			{:else}
				<div class="framily-grid">
					{#each framilies as framily}
						<a href="/framily/{framily.code}" class="framily-card">
							<h3>{framily.name || 'Unnamed Framily'}</h3>
							<p class="code">Code: {framily.code}</p>
							<p class="members">{framily.member_count} member{framily.member_count !== 1 ? 's' : ''}</p>
