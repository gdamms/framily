<script lang="ts">
	...
	$: userRole = framily?.members?.find(m => m.user_id === $authStore.user?.id)?.role ?? -1;
	$: isAdmin = userRole === 2;
	...
	async function deleteFramily() {
		if (!confirm('Are you sure you want to DELETE this framily? This cannot be undone!')) return;
	...
	{:else if framily}
		<header>
			<div class="header-info">
				<h1>{framily.name || 'Unnamed Framily'}</h1>
				<p class="code">Code: <code>{framily.code}</code></p>
			</div>
			<nav class="tabs">
				<button class:active={activeTab === 'gallery'} onclick={() => activeTab = 'gallery'}>
					...
					<div class="gallery-grid">
						{#each pictures as picture}
							<div class="picture-card">
								<img src={picture.url} alt="Uploaded by {picture.uploader_name || 'Unknown'}" />
								<div class="picture-info">
									<span class="uploader">{picture.uploader_name || 'Unknown'}</span>
									<span class="date">{new Date(picture.upload_date).toLocaleDateString()}</span>
									{#if canDeletePicture(picture)}
										<button class="delete-btn" onclick={() => deletePicture(picture.id)}>
											Delete
										</button>
									{/if}
								</div>
						...
						<div class="member-card">
							...
							{#if isAdmin && member.user_id !== $authStore.user?.id && member.role >= 1}
								<div class="member-actions">
									...
									{/if}
									<button class="kick-btn" onclick={() => kickMember(member.username)}>
										Kick
									</button>
								</div>
							{/if}
					...
					<div class="modal" onclick={(e) => e.stopPropagation()}>
						...
						<form onsubmit={(e) => { e.preventDefault(); inviteUser(); }}>
							...
							<div class="modal-actions">
								...
								<button type="submit" disabled={inviteLoading || !inviteUsername}>
			...
			<section class="settings-section">
				...
				<form onsubmit={(e) => { e.preventDefault(); saveSettings(); }}>
					...
					<div class="form-group">
						...
						<select id="transition_effect" bind:value={settingsForm.transition_effect}>
				...
				<div class="danger-zone">
					...
					<p class="warning">This will permanently delete all photos and data.</p>
				</div>
			</section>
		{/if}
	{/if}
