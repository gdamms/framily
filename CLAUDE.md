...
```sh
make setup       # one-time: copies config/env/dev.env -> config/env/.env, adds UID/GID
...
Dev stack service ports/vars come from `config/env/dev.env` (copied to `.env` by
...
- `config/env/*.env` are templates; the real `.env` (dev) / `.env.prod` (prod) are
